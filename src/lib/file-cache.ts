...
export class FileBatchOperations {
  /**
   * Get multiple file stats in parallel
   */
  static async getFileStats(
    filePaths: string[]
  ): Promise<Map<string, { size: number; mtime: Date }>> {
   ...
   */
  static async validateFiles(filePaths: string[]): Promise<string[]> {
    const promises = filePaths.map(async (path) => {
      try {
        await stat(path);
        return path;
