// Client-safe file utilities (no Node.js fs module)
...
export function validateFileType(file: File): {
  ...
  if (!allowedFileTypes.includes(file.type)) {
    return {
      isValid: false,
      error:
        'Desteklenmeyen dosya türü. Sadece PDF, Word, Excel ve resim dosyaları kabul edilir.',
    };
  }
...
export function getFileTypeLabel(mimeType: string | null): string {
  return mimeType ? fileTypeLabels[mimeType] || 'Dosya' : 'Dosya';
}
